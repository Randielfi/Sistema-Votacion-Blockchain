...
contract Election {
    ...
    modifier electionActive(uint electionId) {
        require(
            elections[electionId].started && !elections[electionId].ended,
            "Eleccion no activa"
        );
        _;
    }
    ...
    function vote(
        ...
        require(
            candidateIndex < e.candidates.length,
            "Indice de candidato invalido"
        );
        ...
        emit VoteCasted(
            electionId,
            candidateIndex,
            e.candidates[candidateIndex].name
        );
    }
    ...
    function voteWithProof(
        ...
        // Verificar la prueba zk-SNARK
        require(verifier.verifyProof(a, b, c, input), "Invalid ZK proof");

        // Validar que el electionId en la prueba coincide con el recibido
        require(input.length >= 4, "Input length too short");
        require(input[2] == electionId, "ElectionId mismatch in proof");

        // Validar que el candidateIndex en la prueba coincide con el recibido
        require(input[3] == candidateIndex, "CandidateIndex mismatch in proof");

        // Leer nullifierHash desde public input → input[1]
        ...
        // Marcar nullifierHash como usado
    ...
    function voteFor(
        ...
        uint candidateIndex,
        address voter
    ) public onlyOwner electionExists(electionId) electionActive(electionId) {
        ElectionData storage e = elections[electionId];
        require(
            !e.hasVoted[voter],
            "Esta direccion ya ha votado en esta eleccion"
        );
        require(
            candidateIndex < e.candidates.length,
            "Indice de candidato invalido"
    ...
    function getCandidatesCount(
        uint electionId
    ) public view electionExists(electionId) returns (uint) {
    ...
    function getWinner(
        ...
        view
        electionExists(electionId)
        returns (string memory winnerName, uint winnerVotes, bool isTie)
    {
        ElectionData storage e = elections[electionId];
